...
const getNotes = async () => {
    ...
    const notesDecrypted = await Promise.all(notesEncrypted.map(n => decryptVaultItem(n, key)))
...
const save = async () => {
    if((note.value.title != '' || note.value.note != '') && changed.value) {
        try {
            let res
            const encrypted = await encryptVaultItem({note: note.value.note}, key)
...
const showDesktopContextMenu = (event, item) => {
    contextMenuItems.value[0].command = () => {confirmDelete(item)}
        ...
        <Dialog v-model:visible="noteDialog" modal pt:content:class="!h-full" :style="{ width: '100%', height: '100%' }" @hide="save">
            <template #header>
                <div class="flex flex-col w-full">
                    <InputText size="large" fluid type="text" v-model="note.title" placeholder="Judul" class="!bg-inherit !border-0" @input="onchange" />
                    <div class="font-medium text-surface-500 dark:text-surface-400 text-sm pl-4">{{ dateHandler(note.date, false) }}</div>
                </div>
            </template>
            <Textarea v-model="note.note" placeholder="Mulai mengetik" autofocus class="h-[95%] !bg-inherit !border-0" fluid @input="onchange" />
        </Dialog>
